...
const portfolioSupabase = window.supabase
    ? window.supabase.createClient(PORTFOLIO_SUPABASE_URL, PORTFOLIO_SUPABASE_KEY)
    : null;
...
function getCategoryContainer(category) {
    ...
    const containers = {
        "web-development": "webDevelopmentProjects",
        "brochure-design": "brochureProjects",
        "presentation-design": "presentationProjects",
        "ebook-design": "ebookProjects",
        "instagram-carousel": "carouselProjects",
        "pitch-deck": "pitchDeckProjects"
    };

    const containerId = containers[normalized];
...
function createProjectCard(project) {
    ...
    // Dynamic Image Resolution supporting imageurl, image_url, cover_image_url, and project_images
    const cover =
        project.imageurl ||
        project.image_url ||
        project.cover_image_url ||
        (Array.isArray(project.project_images) && project.project_images.length
            ? project.project_images[0]
            : typeof project.project_images === "string"
    ...
    card.innerHTML = `
        ...
        <div class="portfolio-project-info">
            <p class="portfolio-project-category">${escapeHtml(category)}</p>
            <h3>${escapeHtml(project.title)}</h3>
            <p>${escapeHtml(project.description || "")}</p>

            <a href="project.html?id=${encodeURIComponent(project.id)}" class="portfolio-learn-more">
                ${
                    normalizeCategory(project.category) === "web-development"
                    ? "VIEW PROJECT →"
                    : "LEARN MORE →"
                }
            </a>
...
function displayProjects(projects) {
    ...
    projects.forEach(project => {
        const container = getCategoryContainer(project.category);

        if (!container) {
            console.warn("Unknown project category:", project.category);
            return;
        }
...
async function loadProjectDetails() {
    ...
    if (!projectId) {
        showProjectError("No project was selected.");
        return;
    }

    if (!portfolioSupabase) {
        showProjectError("Database connection unavailable.");
        return;
    }

    try {
        const { data: project, error } = await portfolioSupabase
            .from("projects")
            .select("*")
            .eq("id", projectId)
            .single();
        ...
        if (error || !project) {
            ...
            showProjectError("Project could not be found.");
            return;
        }
        ...
        console.error("Unexpected detail error:", error);
        showProjectError("Something went wrong while loading this project.");
    }
}
...
function displayProjectDetails(project) {
    ...
    if (cover) {
        ...
        if (coverUrl) {
            cover.src = coverUrl;
            cover.alt = project.title || "Project Image";
    ...
    /* --- EXTERNAL LINK --- */
    if (externalLink) {
        const link = project.project_link || project.link;
        if (link) {
    ...
    if (gallery) {
        ...
        } else if (typeof project.project_images === "string" && project.project_images) {
            images = [project.project_images];
        }
    ...
    document.title = `${project.title || "Project"} | Fofana Umar`;
}
